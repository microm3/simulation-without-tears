...
fun launchVisualizer(
    ...
    val commandToRun =
        command
            ...
            ?: throw IllegalArgumentException("The module does not contain any run commands.")

    when (val outcome = solveCommand(module, commandToRun)) {
        is SolveOutcome.Unsat -> {
            println("UNSAT: ${outcome.command}")
            val unsatResult = scenarioResult?.copy(
                naturalLanguage = "$UNSAT_BANNER\n\n${scenarioResult.naturalLanguage}",
            )
            SwingUtilities.invokeLater {
                showLauncherWindow("No instance found (UNSAT)", unsatResult) { close ->
                    listOf(
            ...
            SwingUtilities.invokeLater {
                ...
                showLauncherWindow("Instance Explorer", scenarioResult) { close ->
                    listOf(
                        ...
                        retryButton(close, onAddAnotherConstraint, beforeRetry = {
...
private fun nextInstanceButton(controller: SolutionController): JButton =
    JButton("Next instance").apply {
        addActionListener { isEnabled = controller.showNext() }
    }

private fun openVizGui(xmlPath: Path, theme: Path): VizGUI =
    VizGUI(/* standalone = */ false, /* xmlFileName = */ xmlPath.toString(), /* windowmenu = */ null).also {
        it.loadThemeFile(theme.toString())
        it.doShowViz()
    }

private fun allWorldsButton(xmlPath: Path, theme: Path): JButton =
    JButton("Open another window").apply {
        addActionListener { openVizGui(xmlPath, theme) }
    }
