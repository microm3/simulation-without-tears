...
object PairwiseScenarioRunner {
    ...
    fun main(rawArgs: Array<String>) {
        ...
        val scenarios =
            rawArgs.flag(COMBINATION_POLICY_FLAG)
                ?.let { applyTargetPropertyCombinationPolicy(SimulationScenariosLoader.scenarios, Paths.get(it)) }
                ...
                .map { sc ->
                    sc to
                        satCasesBySc[sc.id].orEmpty().take(casesPerScenario).map { bindings ->
                            val resolved = resolveBindings(sc, ParameterisedScenario(sc.id, bindings), relations)
        ...
        cases.forEach { (sc, combos) ->
            ...
            val note = if (available <= casesPerScenario) "all" else "${combos.size}/$available sampled (limited by valid combinations)"
        ...
        Files.newBufferedWriter(outPath).use { jsonl ->
            for (tuple in tuples) {
                ...
                val row =
                    runPredicate(
                        parameterised = tuple.map { (sc, cb) -> ParameterisedScenario(sc.id, cb.bindings) },
    ...
    private fun tupleClassBindingsOverlap(
        ...
        val sets =
            tuple.map { (sc, cb) ->
                val rendered =
                    ScenarioExpansion.renderScenarioTemplate(sc.alloyTemplate, cb.alloy, modelMetadata)
    ...
    private fun enumerateTuples(
        ...
        fun policyOk(tuple: List<ScenarioDefinition>): Boolean =
            when (policy) {
                ...
                "disallowed" -> {
                    tuple.all { cand ->
                        val tags = cand.conflictGroups.toSet()
                        tuple.all { other ->
                            other.id == cand.id || other.conflictGroups.any(tags::contains)
                        }
                    }
                }
        ...
        return Sets
            .combinations(scenarios.indices.toSet(), size)
            .filter { idxSet -> policyOk(idxSet.sorted().map { scenarios[it].first }) }
            .flatMap { idxSet ->
                val tuple = idxSet.sorted().map { scenarios[it] }
